...
{
    if (t != NULL){ //Goes through each node and deletes from left to right
        if (t->left != NULL){ //Checks if the left is already empty
            makeEmpty(t->left);
        }
        if (t->right != NULL){ //Checks if the left is already empty
            makeEmpty(t->right);
        }
    }
...
{
    if (t == NULL) { //Fresh node and creating the root of the tree
        t = new TreeNode;
        t->data = newItem;
        t->left = NULL;
        t->right = NULL;
    } else {
        if (t->data > newItem) { //If the newItem is less than the current node then it inserts left of the node
            insertItem(t->left, newItem);
        } else if (t->data < newItem){ //If the newItem is greater than the current node then it inserts right of the node
...
{
    insertItem(root, newItem); //Calls the actual function
}
...
{
    int count = 0; // returns zero if the tree is empty

    if(t != NULL){
        count = 1; //At least one node with the root established

        if(t->left != NULL){ //Counts the nodes on the left side
            count = count + countNodes(t->left);
        }
        if(t->right != NULL){ //Counts the nodes on the right side
...
void BST_312 <ItemType>::preOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    if(t == NULL){ //Empty tree
        ...
        result.push_back(t->data); //Pushes back the value into the vector in the beginning for preOrder
        if (t->left != NULL){//Checks if the left is empty
            preOrderTraversal(t->left, result);
        }
        if (t->right != NULL){//Checks if the right is empty
...
vector<ItemType> BST_312 <ItemType>::preOrderTraversal()
{
    ...
    vector<ItemType> preOrderVector;

    preOrderTraversal(root, preOrderVector);//Calls the actual function
...
void BST_312 <ItemType>::inOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    if(t == NULL){ //Empty tree
        ...
        result.push_back(t->data); //Pushes back the value into the vector in the middle for inOrder
...
{
    vector<ItemType> inOrderVector;

    inOrderTraversal(root, inOrderVector); //Calls the actual function
...
void BST_312 <ItemType>::postOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    if(t != NULL) {
        ...
        if (t->right != NULL){ //Checks if the left is empty
            postOrderTraversal(t->right, result);
        }

        result.push_back(t->data); //Pushes back the value into the vector at end for the postOrder
    }
}
...
{
    vector<ItemType> postOrderVector;

    postOrderTraversal(root, postOrderVector); //Calls the actual function
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
    ...
    while(t != NULL){ //Checks the entire tree
        if(t->data == item) //If the value is found return true
            return true;
        else if(item > t->data) //Goes into the right subtree if the item is greater than the current data value
            t = t->right;
        else if(item < t->data) //Goes into the left subtree fi the item is less than the current data value
            t = t->left;
    }
